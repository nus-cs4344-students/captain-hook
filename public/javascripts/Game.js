function Game() {
    ...
    function create() {
        ...
        captain = new Captain(game, 100, 300, 0);
    ...
    function update() {
        ...
        game.physics.arcade.collide(captain.body, platforms);

        if (!captain.hookReturn) {
            ...
            for (var i = 0; i < arrayOfPlayer.length; i++) {
                if (game.physics.arcade.collide(that.hook1, arrayOfPlayer[i].body)) {
                    ...
                    if ((captain.body.x < 270) && (that.hook1.x > 520)) {
                        ledge1.body.checkCollision.right = false;
                        ledge2.body.checkCollision.right = false;
                    }
                    else if ((captain.body.x > 520) && (that.hook1.x < 270)) {
                        ledge1.body.checkCollision.left = false;
                        ledge2.body.checkCollision.left = false;
                    }
                    var back1 = setInterval(function () {
                        game.physics.arcade.moveToObject(that.hook1, captain.body, 200);
                        game.physics.arcade.moveToObject(temp, captain.body, 200);
                        if (game.physics.arcade.distanceBetween(that.hook1, captain.body) < 25) {
            ...
            for (var i = 0; i < arrayOfPillar.length; i++) {
                if (game.physics.arcade.collide(that.hook1, arrayOfPillar[i])) {
                    ...
                    var back2 = setInterval(function () {
                        game.physics.arcade.moveToObject(captain.body, that.hook1, 200);
                        if (game.physics.arcade.distanceBetween(that.hook1, captain.body) < 25) {
                            that.hook1.kill();
                            captain.hookedPillar = false;
                            captain.body.body.velocity.x = 0;
                            captain.body.body.velocity.y = 0;
                            ledge1.body.checkCollision.left = true;
                            ledge2.body.checkCollision.left = true;
                            ledge1.body.checkCollision.right = true;
                            ledge2.body.checkCollision.right = true;
        ...
        var counter = 0;
        for (var i = 0; i < arrayOfPlayer.length; i++) {
            if (game.physics.arcade.distanceBetween(captain.body, arrayOfPlayer[i].body) < 25) {
                if (captain.teamID != arrayOfPlayer[i].teamID) {
                    counter++;
        ...
        if (captain.hook.countLiving() > 0) {
            if (game.physics.arcade.distanceBetween(that.hook1, captain.body) > 500) {
                captain.hookReturn = true;
                game.physics.arcade.moveToObject(that.hook1, captain.body, 200);
                var back3 = setInterval(function () {
                    game.physics.arcade.moveToObject(that.hook1, captain.body, 200);
                    if (game.physics.arcade.distanceBetween(that.hook1, captain.body) < 50) {
                        //console.log("testing");
                        captain.hookReturn = false;
                        that.hook1.kill();
                        clearInterval(back3);
                    }
                }, 10)
        ...
        captain.body.rotation = game.physics.arcade.angleToPointer(captain.body);
        captain.body.body.velocity.x = 0;
        captain.body.body.velocity.y = 0;
        ...
        if (game.input.activePointer.isDown) {
            ...
            captain.body.body.velocity.x -= 100;
        }
            ...
            captain.body.body.velocity.y -= 100;
        }
    ...
    function fire() {
        ...
        if (captain.hook.countLiving() < 1) {
            that.hook1 = captain.hook.getFirstDead();

            //set bullet starting position
            hook1.reset(captain.body.x - 5, captain.body.y - 5);
    ...
    function render(pointer) {
        game.debug.text('Active star: ' + captain.hook.countLiving() + ' / ' + captain.hook.total + " x:" + captain.body.x + "	px:" + game.input.x + " HP:" + captain.hp, 32, 32);
